...
class Observation(Base):
    ...
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    updated_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
    __table_args__ = (UniqueConstraint('country_code', 'indicator_code', 'year', name='uq_observation'),)
...
def bulk_upsert_observations(observations: List[Dict[str, Any]]) -> int:
    """FIX: More robust bulk upsert for observations using SQLAlchemy's dialect-specific insert."""
    ...
    try:
        ...
        # Specify the constraint to check for conflicts
        final_stmt = stmt.on_conflict_do_update(
            constraint='uq_observation',  # Use the actual constraint name from your schema
            set_=update_cols
        )
