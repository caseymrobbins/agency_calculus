...
def load_config(config_path: str = None) -> dict:
    ...
    config_path = config_path or os.getenv('CONFIG_PATH', 'ingestion/config.yaml')
    try:
...
def fetch_world_bank_data_batch(
    ...
    try:
        # THE FIX IS HERE: Changed 'data_date=' to 'date='
        ...
        country_name_to_code_map = {c['name']: c['id'] for c in wbdata.get_countries()}
...
def run_ingestion(config_path: str = None, return_df: bool = False) -> Optional[pd.DataFrame]:
    ...
    try:
        config = load_config()
        countries = config.get('world_bank', {}).get('countries', [])
        indicators_list = config.get('world_bank', {}).get('indicators', [])
        
        if not countries or not indicators_list:
            raise ValueError("Configuration must specify 'countries' and 'indicators' under the 'world_bank' key in the config file.")
        ...
        for obs in all_observations:
            ...
            obs['notes'] = f"Data sourced from World Bank API on {dataset_version}"
