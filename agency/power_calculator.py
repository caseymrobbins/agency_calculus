...
Purpose: Calculates a composite Power score (P) from multiple metrics based on
         the Agency Calculus 4.3 formula. It supports the Adversarial Weighting
...
IDEOLOGICAL_WEIGHTS = {
    "framework_average": {
        "economic": 0.4, "political": 0.3, "social_reach": 0.15, "social_centrality": 0.15
    },
    "libertarian": {
        "economic": 0.6, "political": 0.2, "social_reach": 0.1, "social_centrality": 0.1
    },
    "socialist": {
        "economic": 0.15, "political": 0.25, "social_reach": 0.3, "social_centrality": 0.3
    },
    "communitarian": {
        "economic": 0.25, "political": 0.25, "social_reach": 0.25, "social_centrality": 0.25
    }
}
...
class PowerCalculator:
    ...
    Implements the formula: P = (P'_econ^w_e) * (P'_poli^w_p) * (P'_soc_reach^w_s1) * (P'_soc_centrality^w_s2)
    """
    ...
    def _normalize_metrics(self, metrics: Dict[str, float]) -> Dict[str, float]:
        ...
        for key, value in normalized.items():
            if not (0 <= value <= 1):
                logger.warning(f"Normalized value for '{key}' is outside [0, 1] range: {value}")
            normalized[key] = np.clip(value, 0, 1)
        return normalized

    def calculate_power(self, metrics: Dict[str, float], return_components: bool = False) -> float | Tuple[float, Dict]:
        ...
        component_contributions = {key: self.weights[key] * np.log(p_prime[key] + epsilon) for key in self.weights}
